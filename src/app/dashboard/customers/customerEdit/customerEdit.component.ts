...
export class CustomerEditComponent implements OnInit {
    public modalId = "bugModal";
    public userForm: FormGroup;
    // private statuses = STATUS;
    // private severities = SEVERITY;
    public transactionsCustomers: Transaction[]=[];
    public statusArr: string[] = [];
    public severityArr: string[] = [];
    public currentUser = new Customers(null, null, null, 0, 0, "No", null, [null], null, [null]);

    public BookingsMissed: BookingsMissed[] = [];
    ...
    configureForm(user?: Customers) {
        ...
        if (user) {
            ...
            // this.getMissedBookings(user);
            this.getEachCustomersTransactions(user);
...
getEachCustomersTransactions(user: Customers){
     this.customerService.getEachCustomerTransactions(user)
        .subscribe(user => {
            this.hasbookings = true;
            console.log(user);
            this.transactionsCustomers.push(user);
            console.log(this.transactionsCustomers);
    ...
    //     this.customerService.getMissedBookings(user)
    //     .subscribe(user => {
    //         console.log(user);
    //         this.Bookings.push(user);
    //         console.log(this.Bookings);
