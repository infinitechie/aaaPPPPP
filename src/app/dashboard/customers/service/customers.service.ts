...
getMissedBookings(user?: Customers){
    // var bookingsRef = this.customersRef.child(user.id).child("bookingsMissed")
     ...
     return Observable.create(obs => {
                ...
                this.customersRef.child(user.id).child("stockTransactions").on('child_added', customer => {
...
getEachCustomerTransactions(user?: Customers){
    // var bookingsRef = this.customersRef.child(user.id).child("bookingsMissed")

     return Observable.create(obs => {
            
                this.customersRef.child(user.id).child("transactions").on('child_added', customer => {
