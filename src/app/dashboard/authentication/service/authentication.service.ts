    ...
    // var bookingsRef = this.customersRef.child(user.id).child("bookingsMissed")
     ...
     return Observable.create(obs => {
      ...
      addUser(customer: Staff) {
        ...
        imageStorageRef: customer.imageStorageRef,
        unitsSold: customer.unitsSold,
        dates: [customer.dates],
        productsSold: [customer.productsSold],
        transactions: [customer.transactions], //Epoch date number of when ordered
        servicesList: [customer.servicesList],
        bookingsMissed: [customer.bookingsMissed],
        highlighted: customer.highlighted
        })
        .catch(err => console.error("Unable to add bug to Firebase - ", err));
    }
